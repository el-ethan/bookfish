...
def bookfish(url, codec='gb18030', print_to_file=False):
    ...
    # Get url tail
    # Get html
    html = urllib2.urlopen(url).read().decode(codec)
    # Get title
    ...
    for tail in url_tails:
        url_base = re.sub('\.html?$|book-\d+.shtml|/html/.*opf.html', '', url)
        new_url = url_base + tail
        html = urllib2.urlopen(new_url).read().decode(codec)
        html_novel += '#'*20 + html
