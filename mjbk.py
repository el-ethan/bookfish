...
def decoder_ring(mojibake, output_type='lst'):
    ...
    for encoder in codecs:
        ...
        for decoder in cjk_codecs:
            try:
                ...
                continue
            if (d_moji not in moji.values() and
                d_moji != mojibake):
...
if __name__ == '__main__':
    ...
    mjbk = 'ÄãºÜºÃ'
    moji = decoder_ring(mjbk)
    string_moji = ', '.join(moji)
    # for mono in bakemono:
        # cleaner_string = re.sub('.*%s.*' % mono, '', string_moji)
